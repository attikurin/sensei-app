/**
 * 先生アプリ - 認証状態管理（全ページ共通）
 *
 * このスクリプトはすべてのページの <head> に読み込まれます。
 * Firebase Auth の状態を監視し、ヘッダーのUI（ログイン/ログアウトボタン）を
 * 動的に切り替えます。
 */

import { auth, loginWithGoogle, logout, getUserDoc, onAuthStateChanged }
  from "./firebase.js";

// =============================================
// 認証状態の監視 & ヘッダーUI更新
// =============================================
onAuthStateChanged(auth, async (user) => {
  // ページ全体に現在のユーザーを公開
  window.currentUser    = user;
  window.currentUserDoc = null;

  const loginBtn    = document.getElementById("login-btn");
  const logoutBtn   = document.getElementById("logout-btn");
  const userMenuBtn = document.getElementById("user-menu-btn");
  const userAvatar  = document.getElementById("user-avatar");
  const postBtn     = document.getElementById("post-btn");
  const userNameEl  = document.getElementById("user-name-display");
  const adminLink   = document.getElementById("admin-link");

  if (user) {
    // Firestoreからユーザー情報取得
    const userDoc = await getUserDoc(user.uid);
    window.currentUserDoc = userDoc;

    // BAN チェック
    if (userDoc && userDoc.banned) {
      await logout();
      UI.showToast("このアカウントは利用停止されています。", "error");
      return;
    }

    // ヘッダーUIを「ログイン済み」に切り替え
    if (loginBtn)    loginBtn.classList.add("hidden");
    if (userMenuBtn) userMenuBtn.classList.remove("hidden");
    if (postBtn)     postBtn.classList.remove("hidden");

    // アバター画像
    if (userAvatar) {
      if (user.photoURL) {
        userAvatar.innerHTML = `<img src="${user.photoURL}" alt="${user.displayName}">`;
      } else {
        userAvatar.textContent = (user.displayName || "先")[0];
      }
    }

    // 表示名
    if (userNameEl) userNameEl.textContent = user.displayName || "先生";

    // 管理者リンク
    if (adminLink && userDoc && userDoc.role === "admin") {
      adminLink.classList.remove("hidden");
    }

  } else {
    // 未ログイン
    window.currentUserDoc = null;
    if (loginBtn)    loginBtn.classList.remove("hidden");
    if (userMenuBtn) userMenuBtn.classList.add("hidden");
    if (postBtn)     postBtn.classList.add("hidden");
    if (adminLink)   adminLink.classList.add("hidden");
  }

  // 認証完了イベントを発火（各ページのスクリプトが待機できる）
  document.dispatchEvent(new CustomEvent("authReady", { detail: { user } }));
});

// =============================================
// ログインボタン
// =============================================
document.addEventListener("DOMContentLoaded", () => {
  const loginBtn  = document.getElementById("login-btn");
  const logoutBtn = document.getElementById("logout-btn");

  if (loginBtn) {
    loginBtn.addEventListener("click", async () => {
      try {
        loginBtn.disabled = true;
        loginBtn.textContent = "ログイン中...";
        await loginWithGoogle();
        UI.showToast("ログインしました！", "success");
      } catch (e) {
        UI.showToast("ログインに失敗しました。もう一度お試しください。", "error");
        loginBtn.disabled = false;
        loginBtn.innerHTML = `<svg width="18" height="18" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg> Googleでログイン`;
      }
    });
  }

  if (logoutBtn) {
    logoutBtn.addEventListener("click", async () => {
      await logout();
      UI.showToast("ログアウトしました。", "default");
      window.location.href = "/";
    });
  }
});
