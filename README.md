...
| 修正日 | ファイル | 内容 |
|--------|---------|------|
| 2026-06 | `apps.html` | 検索ワード（URLパラメータ `?search=`）が実際にフィルタリングされない問題を修正。クライアント側フィルター実装（title/description/tags対象） |
| 2026-06 | `apps.html` | タグ切り替え時に検索ワードがリセットされなかった問題を修正 |
| 2026-06 | `apps.html` | 検索中にページネーションが表示される問題を修正 |
| 2026-06 | `mypage.html` | `delete-modal` のオーバーレイ外クリックで閉じない問題を修正 |
| 2026-06 | `mypage.html` | フッターをナビゲーションリンク付き3カラム構成に統一 |
| 2026-06 | `app-detail.html` | `delete-modal` のオーバーレイ外クリックで閉じない問題を修正 |
| 2026-06 | `index.html` | 不要な `ui.js` 動的ロードを削除 |
| 2026-06 | `submit.html` | `#thumb-file-input { display:none }` の二重定義を削除 |
| 2026-06 | `submit.html` / `contact.html` / `terms.html` / `privacy.html` | フッターをナビゲーションリンク付き3カラム構成に統一 |

---

## ❗ 未実装・今後の改善ポイント

- [ ] **全文検索の強化**：現状はクライアント側フィルター（title/description/tags）。Algolia / Typesense 等の導入で精度向上推奨
...
- [ ] **Firebase Storage CORS設定**：HTMLファイルのfetch+Blobダウンロードに必要（`origin: ["https://sensei-app-tau.vercel.app"]`）

---

## 📅 更新履歴

- **2026-06** バグ修正・デザイン統一（検索機能修正・モーダル修正・フッター統一等）
